...
def command_tracker_ui():
    return ui.div(
        ...
        ui.output_ui("cmd_targeting_plan_section"),
...
def command_tracker_server(input, output, session, app_state):
    ...
    def cmd_scorecard_section():
        ...
        try:
            ...
            children.append(ui_helpers.render_dict_table(command_metrics.miss_direction_rows(pitches, throws)))
    ...
    @render.ui
    def cmd_targeting_plan_section():
        _refresh_tick()
        if not _access_ok():
            return None
        bullpen_id = _active_bullpen_id()
        ...
        try:
            ...
            return ui.div(
                ui.hr(),
                ui.h5("Pitch targeting plan", class_="gbo-section-title"),
                ui.p(
                    ...
                    "aims a bit arm side of the true target instead.",
                    class_="text-muted small",
                ),
                ...
                output_widget("cmd_targeting_plan_chart"),
            )
        finally:
            db.close()

    @render_plotly
    def cmd_targeting_plan_chart():
        _refresh_tick()
        if not _access_ok():
            return None
        bullpen_id = _active_bullpen_id()
