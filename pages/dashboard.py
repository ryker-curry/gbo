        ...
        if role_name == "Coach" and specialty in ("Pitching", "Hitting"):
            return ui.div(
                ui.input_radio_buttons(
                    "kpi_window",
                    None,
                    {"season": "Current Season", "last30": "Last 30 Days"},
                    selected="season",
                    inline=True,
                ),
                class_="mb-3",
...
def _game_ids_since(db, since_date):
    return {row[0] for row in db.query(Game.game_id).filter(Game.game_date >= since_date).all()}


def _pitching_pitches_for_window(db, player_id, window, season, game_ids_last30):
    if window == "last30":
        return [p for p in get_pitching_pitches(db, player_id) if p.game_id in game_ids_last30]
...
def _batting_pitches_for_window(db, player_id, window, season, game_ids_last30):
    if window == "last30":
        return [p for p in get_batting_pitches(db, player_id) if p.game_id in game_ids_last30]
...
def _pitching_staff_section(db, players, window):
    ...
    "last30" (rolling 30-day window), driven by the radio toggle in
    controls() above."""
    season = _current_season(db)
    game_ids_last30 = _game_ids_since(db, date.today() - timedelta(days=30)) if window == "last30" else set()

    per_pitcher = []
    all_pitches = []
    for p in players:
        pitches = _pitching_pitches_for_window(db, p.player_id, window, season, game_ids_last30)
        if not pitches:
            continue
...
def _hitter_staff_section(db, players, window):
    ...
    all_pitches = []
    for p in players:
        pitches = _batting_pitches_for_window(db, p.player_id, window, season, game_ids_last30)
        if not pitches:
            continue
