...
Hitting's "Slash Line"/"Situational"/"Zone-Tier Discipline"/"Batted-
Ball Profile" sections are the season-aggregate counterpart to
pages/hitter_game_report.py's single-game report -- same
compute_batting_line()/compute_batted_ball_profile()/
compute_zone_tier_discipline() calls either way. Pull/Center/Oppo % use
the standard 30/30/30-degree spray-angle convention (see
field_location.classify_spray_direction) -- a documented approximation,
not an exact reproduction of any specific published system.
"""
...
from plate_discipline import (
    ...
    render_zone_performance_heatmap, compute_zone_tier_discipline,
)
...
try:
    ...
    if not selected_player.is_pitcher:
        ...
        if not batting_pitches:
            ...
            profile = compute_batted_ball_profile(batting_pitches, bats=selected_player.bats)
