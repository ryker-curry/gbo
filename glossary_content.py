...
METRICS = [
    ...
    ("Est. VAA (Vertical Approach Angle)", "The angle the pitch is on as it crosses the front of the plate, measured down from a flat horizontal line -- more negative (steeper) means it's dropping into the zone more; closer to zero (flatter) means it's playing more like a rising fastball relative to a hitter's swing plane. GBO estimates this from release height, release angle, and extension rather than measuring it directly off the plate crossing, so treat it as a useful relative comparison (pitch to pitch, pitcher to pitcher) rather than an exact number -- it's labeled \"Est.\" everywhere it's shown for that reason."),
...
TUNNELING = [
    ...
    ("Ratio", "Plate / Tunnel. Higher means the two pitches looked more alike through the decision point and then diverged more afterward -- more deceptive. This is GBO's own metric -- despite the similar name, it is NOT Baseball Prospectus's published \"Break:Tunnel Ratio\" (checked directly against BP's own definition, which is Break Differential / Tunnel Differential, not Plate/Tunnel). See Break:Tunnel % (BP) below for the field that actually matches BP's formula and their published benchmark."),
    ("Break:Tunnel % (BP)", "Late Break / Tunnel, as a percentage -- this IS the literal formula Baseball Prospectus published as their \"Break:Tunnel Ratio.\" Reference: their 2017 MLB-wide study found a league average of 27.6%; their example pitcher, Jon Lester, sat at 28.3%. Same comparability caveats as Tunnel above (fixed-distance vs. fixed-time measuring point, MLB vs. Division II). Context only, shown alongside GBO's own Ratio field above -- not folded into Tunneling+'s grade."),
    ("Release (Combined)", "Straight-line separation (in inches) between the two pitches' real release points, combining both release height and release side into one number. A pitcher releasing two pitch types from visibly different slots is telegraphing before the ball even leaves his hand, no matter how well the flight paths converge afterward. Reference: BP's study found a league average of about 2.4\" here; their most consistent pitcher in that sample, Jon Lester, sat at 1.2\"."),
    ("Release Height Diff / Release Side Diff", "The same release separation broken into its two axes -- how far apart the two release points are just vertically (Release Height Diff) vs. just horizontally (Release Side Diff). Shown for context/coaching only, so a fix can be aimed at the right axis -- Tunneling+ still grades on the single combined Release number above, not these two individually. No published reference exists for this specific breakdown -- BP's own study only reported the combined figure above."),
    ("Tunneling+", "GBO's own grade for this pitch pair (100-point scale, 100 = team average, 10 points = 1 standard deviation) -- a 50/50 blend of Ratio (higher is better) and the combined Release number (lower is better), each compared against the rest of the team for that same pitch pair. A transparent, equal-weight placeholder, not a fitted or externally validated weighting. Needs at least 8 real back-to-back pitch sequences of this exact pitch pair to show at all -- shows as \"--\" below that, rather than a grade built on too few pitches. No outside reference exists for this number -- it's a GBO invention, so \"good\" only means relative to the rest of this team."),
    ("Velo Diff / Vert Break Diff", "Velocity and vertical-break differences between the two pitches. Shown as separate context only, NOT folded into Tunneling+ -- speed and shape separation can predict whiffs on their own, independent of how well the pitches actually tunnel together. No numeric good/bad threshold has been published for this -- seemagnus.com's article (the source for including these) established the directional relationship, not a cutoff."),
]
...
ARSENAL = [
    ...
    ("A3P %", "\"Ahead after 3 pitches\": of the plate appearances where the count after exactly 3 pitches is actually knowable -- a real 4th pitch was thrown, or the PA ended on the 3rd pitch via a strikeout -- how many had the pitcher ahead in the count at that point. A PA that instead ended on the 3rd pitch via a ball in play, walk, or HBP has no clean 'count after 3 pitches' to check (GBO stores the count before each pitch, not after), so it's excluded rather than guessed."),
